...
class CombinationsImp(outer: Combinations)(implicit p: Parameters) extends LazyRoCCModuleImp(outer){
    ...
    val fixedWeightModule = LazyModule(new FixedWeight())
    ...
    io.resp.bits.data := MuxLookup(function, FixedWeight.io.out,
            Array(0.U->FixedWeight.io.out, 1.U->Lexicographic.io.out))
...
class Lexicographic()(implicit p: Parameters) extends Submodule {
    ...
    io.out := Mux((result >> length) & 1.U === 1, ~(0.U), result)
}

//Base class for this accelerator's submodules
abstract class Submodule()(implicit p: Parameters) extends LazyModule {
...
class CombinationsRocketConfig extends Config(
    ...
    new fixedWeightCombinations.WithCombinations ++
